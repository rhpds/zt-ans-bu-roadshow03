...
+
Navigate to the `Webapp` tab in your lab environment. You should see the application is online and working correctly.
...
* **N**: Network Nullifier
+
image::sean.png[S.E.A.N - The Co-Worker Simulator, opts="border"]
...
+
image::where-token.png[Location of the Tokens tab for a user, opts="border"]
+
...
In addition it is recommended to copy the token using right-click and dont rely on the copy function in the lab.
...
+
image::token-sean.png[Pasting the API token into S.E.A.N, opts="border"]
...
+
image::break-web.png[Break Web-Application job running, opts="border"]

. **Verify the application is broken.**
+
Once the job completes, refresh the `Webapp` tab. You should now see an error message.
+
...
If you have encountered an issue or have noticed something not quite right, please link:https://github.com/ansible/workshops/issues/new?labels=controller-101&title=Issue+with+Roadshow03&assignees=nmartins0611[open an issue on GitHub].
